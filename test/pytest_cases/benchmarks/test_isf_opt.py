...
class S4RealData(object):
    """
    S4取自真实情况  σ μ ν τ σ’ μ’ ν’ τ’
    # # S4 ISF
    # 取σ=[3, 1] μ=[2, 2]           --> ν可选(3, 1)(2, 1, 1)
    # 取ν=[3, 1] τ=1                --> ν’可选(3)(2, 1)
    # 取ν’=[2, 1] τ’=1              --> σ’μ’组合有([3][2, 1])和([2, 1][2, 1])
    # 既{((3,), (2, 1), 1): 0.4999999999999999, ((2, 1), (2, 1), 1): 0.4999999999999999}
    # 取σ’μ’=[3][2, 1]
    # 同时，σ’μ’=[2, 1][2, 1]也会被一起算出来
    ##
    # # S3 CGC
    # 取σ’μ’=[3][2, 1] ν’=[2, 1]τ’=1 m=1
    # {(1, 1): 1.0, 'N': 1.0}
    # 同时，取σ’μ’=[2, 1][2, 1] ν’=[2, 1]τ’=1 m=1
    ...
    def __init__(self):
        ...
        self.cgc_st_dict_1 = {(1, 1): 1.0, 'N': 1.0}  # 取σ’μ’=[3][2, 1] ν’=[2, 1]τ’=1 m=1
        self.cgc_st_dict_2 = {(1, 1): 0.4999999999999999, (2, 2): -0.4999999999999999, 'N': 0.9999999999999998}
        # 将S3的m'，对应到S4的m
        self.cgc_dict_1 = {(1, 1): 1.0, 'N': 1.0}  # 取σ’μ’=[3][2, 1] ν’=[2, 1]τ’=1 m=1
