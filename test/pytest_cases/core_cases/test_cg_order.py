...
class TestCGOrder(object):

    def setup_class(self):
        self.protector = DBProtector(cgc_rst_folder, extension_name=".test_cg_order_protected")
        ...
        # by old data
        # 格式(s_n, yd_1, yd_2, cg_order_list)
        ...
        # by books
        # 格式(yd_1_tuple, yd_2_tuple, bool_tuple, cg_order_list, s_n)
        ...
        _, self.s_n_finish_file_name = get_cg_order_finish_s_n_name()
        _, self.s_n_finish_full_file_name = get_cg_order_finish_s_n_name(is_full_path=True)
    ...
    def test_001_create_cg_order(self):
        ...
        flag, cg_order = load_cg_order(1, [1], [1], is_flag_true_if_not_s_n=True)
        assert flag
        assert cg_order is False
        flag, cg_order = load_cg_order(1, [1], [1], is_flag_true_if_not_s_n=False)
        assert not flag
        assert isinstance(cg_order, str)
        flag, finish_s_n = get_cg_order_finish_s_n()
        assert flag
        assert finish_s_n == 0

        # check create_cg_order_s_n_1
        flag, finish_s_n = create_cg_order(1)
        assert flag
        assert finish_s_n == 1
        ...
        for i in self.cgo_case_num_list:
            ...
            flag, data = CGOrderInfo(1).query_by_file_name(file_name)
            assert flag
            assert isinstance(data.get("create_time"), str)
            self.create_time_dict["S1"] = data.get("create_time")
            _, full_file_name = get_cg_order_file_name(s_n, yd_1, yd_2, is_full_path=True)
            _, full_finish_file_name = get_cg_order_finish_s_n_name(is_full_path=True)
            for ex in [".pkl", ".txt"]:
                assert os.path.exists(full_file_name + ex)
                assert os.path.exists(full_finish_file_name + ex)
            flag, cg_order = load_cg_order(s_n, yd_1, yd_2, is_flag_true_if_not_s_n=True)
            assert flag
            assert all(cg_order == np.array(cgo_list))
        ...
        flag, data_txt = CGOrderInfo(0).query_txt_by_file_name(finish_file_name)
    ...
    def test_002_create_cg_order_s_n_2_to_4(self):
        ...
        flag, finish_s_n = create_cg_order(4)
        assert flag
        assert finish_s_n == 4

        _, file_name = get_cg_order_file_name(1, [1], [1])
        flag, data = CGOrderInfo(1).query_by_file_name(file_name)
        assert flag
        assert isinstance(data.get("create_time"), str)
        ...
        for i in self.cgo_book_num_list:
            ...
            for yd_1, yd_2, bool_value in zip(yd_1_tuple, yd_2_tuple, bool_tuple):
                ...
                flag, cg_order = load_cg_order(s_n, yd_1, yd_2, is_flag_true_if_not_s_n=True)
                assert flag
                assert all(cg_order == np.array(cgo_list)), "cg_order={}, answer_list={}".format(cg_order, cgo_list)
    ...
    def test_003_create_cg_order_s_n_5_to_9(self):
        # 准备前文
        ...
        # check create_cg_order_s_n_5_to_9
        flag, finish_s_n = create_cg_order(9)
        assert flag
        assert finish_s_n == 9
