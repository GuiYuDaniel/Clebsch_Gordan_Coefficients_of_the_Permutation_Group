...
def create_cg_order(s_n: int=default_s_n):
    """
    ...
    logger.info("#### create_cg_order get input s_n={}".format(s_n))
    start_time_c = time.time()

    # 先查询数据库中完成到S几：如果输入s_n未计算，直接从循环中cut掉算好的部分；如果s_n被计算过了，则给出完成标记（注意不是返回结果）
    flag, finish_s_n = get_cg_order_finish_s_n()
    if not flag:
        err_msg = "get cg_order finish_s_n meet error with msg={}".format(finish_s_n)
        logger.error(err_msg)
        return False, err_msg
    if s_n <= finish_s_n:
        # 说明以前算过了
        msg = "s_n={} cg_order had been calculated, return True, s_n".format(s_n)
        logger.info(msg)
        return True, s_n
    else:
    ...
    for s_i in range(finish_s_n + 1, s_n + 1):  # 循环体为[finish_s_n+1, finish_s_n+2, ..., s_n]
        ...
        if not flag:
            err_msg = "get young_diagrams db for create_cg_order meet error with s_i={}, msg={}".format(s_i, yd_list)
            logger.error(err_msg)
            return False, err_msg
        flag, characters_and_gi_dict = load_characters_and_gi(s_i, is_flag_true_if_not_s_n=False)
        if not flag:
            err_msg = "get characters_and_gi_dict for create_cg_order meet error with s_i={}, msg={}".format(
        ...
        for yd_left in yd_list:  # [σ]
            ...
            for yd_right in yd_list:  # [μ]
                single_cg_order_start_time = time.time()
        ...
        flag, msg = save_cg_order_finish_s_n(s_i, s_i_speed_time, is_check_add_one=True)
        if not flag:
            err_msg = "save_cg_order_finish_s_n meet error with s_i={}, msg={}".format(s_i, msg)
            logger.error(err_msg)
            return False, err_msg

    c_time = time.time() - start_time_c
    logger.info("#### create_cg_order s_n from {} to {} done, return True, finish_s_n={}, "
                "using time={}s".format(finish_s_n + 1, s_n, s_n, c_time))
    return True, s_n


def save_cg_order(s_n: int, yd_1: list, yd_2: list, cg_order: np.ndarray, speed_time: int):
    """
    CG序列的落盘格式为：
    ...
    if not isinstance(yd_1, list) or not isinstance(yd_2, list):
        ...
        logger.error(err_msg)
        return False, err_msg
    if not isinstance(cg_order, np.ndarray):
        err_msg = "get yd_list={} with type={} must be np.ndarray".format(cg_order, type(cg_order))
        logger.error(err_msg)
        return False, err_msg
    ...
    db_info = CGOrderInfo(s_n)
    _, file_name = get_cg_order_file_name(s_n, yd_1, yd_2)
...
def save_cg_order_finish_s_n(s_n: int, s_n_speed_time: int, is_check_add_one=False):
    ...
    flag, finish_s_n_before = get_cg_order_finish_s_n()
    if not flag:
    ...
    _, finish_file_name = get_cg_order_finish_s_n_name()
...
def get_cg_order_finish_s_n():
    """
    flag表示是否有报错，
    finish_s_n表示当前计算完成的Sn，如果没有，则finish_s_n = 1
    """
    _, finish_file_name = get_cg_order_finish_s_n_name()
    flag, data = CGOrderInfo(0).query_by_file_name(finish_file_name)
    if not flag:
        err_msg = "get cg_order finish_s_n meet error with finish_file_name={}".format(finish_file_name)
        logger.error(err_msg)
        return False, err_msg
    if data is False:
...
def load_cg_order(s_n: int, yd_1: list, yd_2: list, is_flag_true_if_not_s_n=True):
    """
    ...
    if not flag:
        err_msg = "cannot query cg_order with s_n={}, file_name={} because {}".format(s_n, file_name, data)
        logger.error(err_msg)
        return False, err_msg
    ...
    if data:
        ...
        if isinstance(cg_order_array, np.ndarray) and cg_order_array.size > 0:
            ...
            err_msg = "cg_order_array queried from db, but cannot get it from data" \
                      "with data={}, cg_order_array={} from db".format(data, cg_order_array)
            logger.error(err_msg)
            return False, err_msg
    else:
        if is_flag_true_if_not_s_n:
            ...
            err_msg = "query not exist cg_order_array db with s_n={}, file_name={}, err_msg={}".format(
                s_n, file_name, data)
            return False, err_msg


def calc_single_cg_order():
    """
    本例整体计算，没有single
    """
    pass
