    ...
    for s_i in range(finish_s_n + 1, s_n + 1):  # 循环体为[finish_s_n+1, finish_s_n+2, ..., s_n]
        ...
        for σ, μ in product(data_si.yd_list, repeat=2):  # [σ], [μ]双循环
            ...
            for ν_st in data_st.yd_list:  # [ν']循环
                ...
                1，计算新的σμν组合；(ISF/CGC)
                               ~~    ~~  ~ ~
                2，利用性质(4)，算σμν, σμν, σμν；(ISF/CGC)
                3，利用性质(2)，算μσν；（注意：μσν'全自对称的，不能跳过，要自己检查自己）(ISF/CGC)
                               ~~    ~~  ~ ~
                4，利用性质(4)，算μσν, μσν, μσν；(ISF/CGC)
                性质2：4-196b
                性质4：4-196d
                '''
                # 1，计算新的σμν组合(ISF)
                ...
                if is_calc_ed:
                    continue  # 跳过被对称性计算过的那些
                logger.debug("new combination σ={}, μ={}, ν_st={}".format(σ, μ, ν_st))
        ...
        for i, ν_β in enumerate(ν_β_list):
            ...
            for m_ν_st in range(1, max_m_ν_st + 1):
                ...
                for σ_μ_β_all_st, isf_square in zip(σ_μ_β_all_st_tuple_list, isf_square_vector):
                    # TODO 这个循环里的load_cgc肯定重复了，它和ν_β无关
                    ...
                    for (m_σ_st, m_μ_st), cgc_st_square in cgc_st_square_dict.items():
                        ...
                        if cgc_key not in cgc_square_dict:
                            ...
                            if single_cgc_square != 0:
                                cgc_square_dict[cgc_key] = isf_square * cgc_st_square / cgc_st_square_n
                        else:
                ...
                if cgc_square_n != 1:
                    ...
                    logger.error(err_msg)
                    return False, err_msg
                cgc_square_dict["N"] = cgc_square_n  # TODO 未来甚至不需要N
